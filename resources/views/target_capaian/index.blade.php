...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-3">
            ...
            @endif
            <div>
                <form action="{{ route('target_capaian.index') }}" method="GET" class="m-0">
                    <div class="input-group">
                        <input name="dimensi" value="{{ request('dimensi') }}" type="text" class="form-control"
                            placeholder="dimensi" aria-label="Recipient's username" aria-describedby="button-addon2">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse ($targetCapaian as $item)
                                <tr class="align-top">
                                    <td>{{ ($targetCapaian->currentPage() - 1) * $targetCapaian->perPage() + $loop->iteration }}
                                    </td>
                                    <td>{{ $item->dimensi }}</td>
                                    <td>{{ $item->elemen }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <a href="{{ route('target_capaian.show', $item->id_target_capaian) }}"
                                                class="btn btn-outline-info" title="Lihat Profil">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="{{ route('target_capaian.edit', $item->id_target_capaian) }}"
                                                class="btn btn-outline-success">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <form id="myForm"
                                                action="{{ route('target_capaian.destroy', $item->id_target_capaian) }}"
                                                method="POST">
                                                @method('DELETE')
                                                @csrf
