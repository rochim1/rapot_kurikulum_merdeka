    ...
    <div class="card-body">
        <form action="{{ route('kelas.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('id_guru') is-invalid @enderror" id="id_guru" name="id_guru" required>
                    ...
                    @foreach($guru as $item_guru)
                        <option value="{{ $item_guru->id_guru }}" {{ old('id_guru') == $item_guru->id_guru ? 'selected' : '' }}>{{ $item_guru->nama }}</option>
                    @endforeach
                </select>
                @error('id_guru')
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('id_tahun_ajaran') is-invalid @enderror" id="id_tahun_ajaran" name="id_tahun_ajaran" required>
                    ...
                    @foreach($tahunAjaran as $item_tahunAjaran)
                        <option value="{{ $item_tahunAjaran->id_tahun_ajaran }}" {{ old('id_tahun_ajaran') == $item_tahunAjaran->id_tahun_ajaran ? 'selected' : '' }}>{{ $item_tahunAjaran->nama_tahun_ajaran }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nama_kelas') is-invalid @enderror" id="nama_kelas" name="nama_kelas" value="{{ old('nama_kelas') }}" maxlength="50" required>
                @error('nama_kelas')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
