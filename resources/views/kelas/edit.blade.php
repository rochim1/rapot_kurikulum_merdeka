        ...
        <form action="{{ route('kelas.update', $kelas->id_kelas) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('id_guru') is-invalid @enderror" id="id_guru" name="id_guru" required>
                    ...
                    @foreach($guru as $item_guru)
                        <option value="{{ $item_guru->id_guru }}" {{ old('id_guru', $kelas->id_guru) == $item_guru->id_guru ? 'selected' : '' }}>
                            {{ $item_guru->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('id_tahun_ajaran') is-invalid @enderror" id="id_tahun_ajaran" name="id_tahun_ajaran" required>
                    ...
                    @foreach($tahunAjaran as $item_tahunAjaran)
                        <option value="{{ $item_tahunAjaran->id_tahun_ajaran }}" {{ old('id_tahun_ajaran', $kelas->id_tahun_ajaran) == $item_tahunAjaran->id_tahun_ajaran ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('tingkat') is-invalid @enderror" id="tingkat" name="tingkat" required>
                    ...
                    <option value="4" {{ old('tingkat', $kelas->tingkat) == '4' ? 'selected' : '' }}>4</option>
                    <option value="5" {{ old('tingkat', $kelas->tingkat) == '5' ? 'selected' : '' }}>5</option>
                    <option value="6" {{ old('tingkat', $kelas->tingkat) == '6' ? 'selected' : '' }}>6</option>
                </select>
                @error('tingkat')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
