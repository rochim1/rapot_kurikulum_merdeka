                                    ...
                                    <tbody>
                                        @foreach ($rapot_nilai as $rapot)
                                            <tr>
                                                ...
                                                <td width="12%">
                                                    {{-- {{ gettype($rapot->rapotNilai) }} --}}

                                                    {{-- @if (is_object($rapot->rapotNilai) && method_exists($rapot->rapotNilai, 'first'))
                                                        {{ dump($rapot->rapotNilai->first()) }}
                                                    @elseif (is_array($rapot->rapotNilai) && isset($rapot->rapotNilai[0]))
                                                        {{ dump($rapot->rapotNilai[0]) }}
                                                    @else
                                                    ...
                                                    <input min="0" max="100" type="number" class="form-control"
                                                        name="nilai_akhir[{{ $rapot->siswa->id_siswa }}]"
                                                        value="{{ optional($rapot->rapotNilai->first())->nilai_akhir ?? '' }}"
                                                        placeholder="0-100">
                                                </td>
                                                ...
                                                <td>
                                                    <div class="form-check">
                                                        @foreach ($tujuan_pembelajaran as $tujuan)
                                                            <input class="form-check-input tujuan-tercapai" type="checkbox"
                                                                @checked(in_array(
                                                                        $tujuan->id_tujuan_pembelajaran,
                                                                        json_decode($rapot->rapotNilai[0]->tujuan_pembelajaran_tercapai ?? '[]', true)))
                                                    ...
                                                    <div class="form-check">
                                                        @foreach ($tujuan_pembelajaran as $tujuan)
                                                            <input class="form-check-input tujuan-tidak-tercapai"
                                                                type="checkbox" @checked(in_array(
                                                                        $tujuan->id_tujuan_pembelajaran,
                                                                        json_decode($rapot->rapotNilai[0]->tujuan_pembelajaran_tidak_tercapai ?? '[]', true)))
