        ...
        <div class="d-flex gap-3">
            @if (Auth::user()->hasRole('admin'))
                ...
                <a href="{{ route('siswa.create') }}" class="btn btn-primary btn-icon-split">
                    <span class="text"><i class="bi bi-plus"></i></i> </span>
                </a>
            @endif
            <form action="{{ route('siswa.index') }}" method="GET" class="m-0">
                <div class="d-flex">
                    <div class="input-group mx-1">
                        <input name="nis" value="{{ request('nis') }}" type="text" class="form-control"
                            placeholder="nis" aria-label="Recipient's username" aria-describedby="button-addon2">
                    </div>
                    <div class="input-group mx-1">
                        <input name="nisn" value="{{ request('nisn') }}" type="text" class="form-control"
                            placeholder="nisn" aria-label="Recipient's username" aria-describedby="button-addon2">
                    </div>
                    <div class="input-group mx-1">
                        <input name="nama" value="{{ request('nama') }}" type="text" class="form-control"
                            placeholder="nama" aria-label="Recipient's username" aria-describedby="button-addon2">
                            ...
                            <tr class="align-top">
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('siswa.show', $item_siswa->id_siswa) }}"
                                            class="btn btn-outline-info" title="Detail">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        @if (Auth::user()->hasRole('admin'))
                                            <a href="{{ route('siswa.edit', $item_siswa->id_siswa) }}"
                                                class="btn btn-outline-success" title="Edit">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>

                                            <form id="myForm"
                                                action="{{ route('siswa.destroy', $item_siswa->id_siswa) }}" method="POST"
                                                title="Hapus">
                                                @method('DELETE')
                                                @csrf
