    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    @forelse ($kelolakelas as $item_kelola_kelas)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $item_kelola_kelas->nama_kelas }}</td>
                            <td>{{ $item_kelola_kelas->guru->nama }}</td>
                            <td>{{ $item_kelola_kelas->tahunAjaran->nama_tahun_ajaran }}</td>
                            <td>{{ $item_kelola_kelas->tingkat }}</td>
                            <td>{{ $item_kelola_kelas->fase }}</td>
                            <td class="d-flex gap-2">
                                <a href="{{ route('kelas.edit', $item_kelola_kelas->id_kelas) }}" class="btn btn-outline-success">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form id="myForm" action="{{ route('kelas.destroy', $item_kelola_kelas->id_kelas) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('import_kelola_kelas') }}" method="post" enctype="multipart/form-data">
                @csrf
                    ...
                    <tbody>
                        @forelse ($kelolakelas as $item_kelola_kelas)
                            <tr>
                                ...
                                <td>{{ $item_kelola_kelas->fase }}</td>
                                <td>
                                    @if ($item_kelola_kelas->siswa->count())
                                        <ul class="list-unstyled mb-0">
                                            @foreach ($item_kelola_kelas->siswa as $siswa)
                                                <li> - {{ $siswa->nama }}</li>
                                            @endforeach
                                        </ul>
                                    @else
                                ...
                                <td>
                                    ...
                                    @if($item_kelola_kelas->is_active == 1)
                                        <span class="badge bg-success">Aktif</span>
                                    @else
                                        <span class="badge bg-danger">Tidak Aktif</span>
                                    @endif
                                
                                    <form action="{{ route('kelas.toggleStatus', $item_kelola_kelas->id_kelas) }}" method="POST" class="d-inline-block">
                                        @csrf
                                        <select name="is_active" class="form-select form-select-sm" onchange="this.form.submit()">
                                            <option value="1" {{ $item_kelola_kelas->is_active == 1 ? 'selected' : '' }}>Aktif</option>
                                            <option value="2" {{ $item_kelola_kelas->is_active == 2 ? 'selected' : '' }}>Tidak Aktif</option>
                                        </select>
                                    </form>                                    
                                </td>                                                             
                                <td class="d-flex gap-2">
