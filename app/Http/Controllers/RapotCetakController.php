...
class RapotCetakController extends Controller
{
            ...
            foreach ($rapot as $item) {
                ...
                foreach ($item->rapotNilai as $nilai) {
                    ...
                    if (is_array($tujuan_ids_tercapai)) {
                        ...
                        $nilai->tujuan_pembelajaran_tercapai_text = $nilai->tujuan_pembelajaran_tercapai_detail
                            ->pluck('tujuan_pembelajaran')
                            ->map(fn($tujuan) => 'Siswa mampu ' . $tujuan)
                            ->join(', ');
                    }
                    ...
                    if (is_array($tujuan_ids_tidak_tercapai)) {
                        ...
                        // Convert to readable format (text) if needed
                        $nilai->tujuan_pembelajaran_tidak_tercapai_text = $nilai->tujuan_pembelajaran_tidak_tercapai_detail->pluck('tujuan_pembelajaran')
                        ->map(fn($tujuan) => 'Siswa tidak mampu ' . $tujuan)
                        ->join(', ');;
                    }
                }
            }
    ...
    public function mergePdfs($rapot, $profilSekolah, $viewPath)
    {
        ...
        foreach ($rapot as $data) {
            ...
            $pdfContent = Pdf::loadView($viewPath ?? 'rapot_cetak.cetak.view_persiswa', $viewData)
                ->setOptions(['isHtml5ParserEnabled' => true, 'isPhpEnabled' => true])
    ...
    public function rapot_all(Request $request)
    {
        if ($request->download_all == 'rapot_umum') {
            ...
            $profilSekolah = ProfilSekolah::find(1);
            return $this->mergePdfs($rapot, $profilSekolah, 'rapot_cetak.cetak.view_persiswa');
            ...
            $profilSekolah = ProfilSekolah::find(1);
            return $this->mergePdfs($rapot, $profilSekolah, 'rapot_cetak.cetak_p5.view_persiswa');
        }
    }
}
